...
$query = "SELECT p.id, u.username AS user_name, p.name AS project_name, p.approval, 
                 p.status, GROUP_CONCAT(up.file_path) AS photo_paths, up.category
          FROM projects p
    ...
    ?>">
                    ...
                    <td class="p-4">
                        <p class="p-4 text-center">
                            <?php echo !empty($row['category']) ? htmlspecialchars($row['category']) : "<span class='text-gray-500'>Kategori belum dipilih</span>"; ?>
                        </p>
                        <?php
